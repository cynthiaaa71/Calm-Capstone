...
// Function to update the user's quiz results in Firestore
async function updateQuizResult(userid, quizName, score) {
...
export async function handleQuizResults(quizName, score) {
  const auth = getAuth();
  const user = auth.currentUser;

  if (!user) {
    console.log(`Your score for ${quizName} is: ${score}`);
    return;
  }

  await updateQuizResult(user.uid, quizName, score); // Call update function
}
